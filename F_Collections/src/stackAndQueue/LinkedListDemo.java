...
//		queueDemo2();
	...
	static void queueDemo2() {
		...
		// auslesen und in der Queue belassen
		String elem = tasks.peek();
		System.out.println("Erstes Element: " + elem);

		// alle Elemente abholen
		while (tasks.size() > 0) {
			elem = tasks.remove();
			System.out.println("Aus Queue abgeholt: " + elem);
		}

		// versuchen, ein Element abzuohlen
		// liefert jetzt null, da kein Element mehr da ist
		elem = tasks.peek();
		System.out.println("Erstes Element: " +
				(elem == null ? "nicht vorhanden" : elem.toString()));

		// würde exception werfen
		elem = tasks.element();
