...
public class PersonenMain {
	...
	//statt Personengruppe
	//private static PersonenGruppe meineGruppe = new PersonenGruppe(5);
	private  static GenerischeListe<Person> meineGruppe = new GenerischeListe<>(5);
	public static void main(String[] args) {
		...
		try{
			// statt anzeigen
			System.out.println(meineGruppe.elementAnIndex(3));
			System.out.println(meineGruppe.elementAnIndex(99));
		}catch (IndexOutOfBoundsException e){
			System.out.println("Fehler beim Abrufen: " + e.toString());
		}
	...
	static void testeAbmeldung(int nr) {
		try {
			Person tmpPerson = meineGruppe.entfernen(nr);
