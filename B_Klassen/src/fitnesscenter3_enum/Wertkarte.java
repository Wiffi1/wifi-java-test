...
*/
    public void abbuchen(Leistungen leistung){
        double betrag = switch (leistung){
            case GYMNASTIK -> 7.0;
            case FITNESS_2H -> 14.0;
            case FITNESS_TAG -> 19.0;
            case GYMNASTIK_SPEZIAL -> 9.0;
            case WELLNESS -> 15.0;
            default -> throw new IllegalArgumentException("Unbekannte Leistung");
        };

        System.out.printf("Abbuchen: %.2f für Leistung %s\n", betrag, leistung);
        if (guthaben - betrag < 0) {
            //System.out.println("Abbuchen " + typ + ": Guthaben zu klein");
            // statt Ausgabe: Fehler werfen
            throw new IllegalArgumentException(
                    "Guthaben zu klein für Abbuchung von " + leistung.toString());
        }
        guthaben -= betrag;
